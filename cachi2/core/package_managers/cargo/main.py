...
def _resolve_main_package(package_dir: RootedPath) -> dict:
    ...
    # versioned, so the version will be pinned artificially to a default value.
    except KeyError:
        return {
...
def _verify_repository_is_locked_or_fail(package_dir: RootedPath) -> None:
...
def _resolve_cargo_package(
    ...
    _verify_repository_is_locked_or_fail(package_dir)
